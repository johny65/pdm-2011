...
public class BaseDatos extends MIDlet implements CommandListener {

	private Display pantalla;
	private List main, vista;
	private TextBox nuevo;
	private Command salir, volver, add;
	...
	public void startApp() {
		...
		try {
			//RecordStore.deleteRecordStore("RSEj2"); //para vaciar el recordstore descomentar
			rs = RecordStore.openRecordStore("RSEj2", true, RecordStore.AUTHMODE_ANY, true);
		} catch (Exception e) {
			System.err.println("Error al abrir el RecordStore");
			destroyApp(true);
		}
		vista = new List("Records almacenados", List.IMPLICIT);
